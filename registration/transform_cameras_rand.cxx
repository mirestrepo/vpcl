...
//  vul_arg<const char *> output_dir ("-out_dir", "directory to store /cams_KRT dir and the file with the applied transformation", "");
  vul_arg<const char *> base_dir ("-base_dir","directory to the differen trials","");
  vul_arg<double> dt ("-dt", "the translation will be uniformly sampled from [-5*dt, 5*dt)");
  vul_arg<int> nt("-nt", "number of trials");
  ...
  for(unsigned trial = 0; trial < nt(); trial++)
  {
    stringstream trial_ss;
    trial_ss << trial;
    vcl_string input_dir = base_dir() + string("/original");
    ...
    vcl_string output_dir = base_dir() + string("/trial_") + trial_ss.str();
